...
def SplinesFig(y, init, end):
    x = np.linspace(init, end, 400)
    plt.axis([0, 7, 0, 9])
    plt.plot(x, y, linewidth=2)
    plt.grid(True)
    plt.show()

def setPoints(xVal, yVal):
    tempY = []
    for i in range(2):
...
def spline(xVal, yVal, ii):
    def S(x):
        ...
        return Sx
    return S
#append value of y to a list
def calAi(yVal):
    tempAi = []
    for i in range(len(yVal)-1):
        tempAi.append(yVal[i])
    return tempAi
...
def main():
    ...
    xVal = [ [1, 2, 3,4], [4,5, 6, 7]]
    yVal = [ [6, 7, 1,1], [1, 8, 7,5]]

    for i in range(2):
        finalYdata += setPoints(xVal[i], yVal[i])
